...
def evaluate_crowd_no_overlap(model, data_loader, device, vis_dir=None):
    ...
    for samples, targets in data_loader:
        ...
        outputs_scores_2 = torch.nn.functional.softmax(outputs['pred_logits'], -1)[:, :, 2][0]
        
        outputs_points = outputs['pred_points'][0]

        gt_cnt = targets[0]['point'].shape[0]
        gt_cnt_1 = len(targets[0]["labels"][targets[0]["labels"]==1])
        gt_cnt_2 = len(targets[0]["labels"][targets[0]["labels"]==2])
        ...
        # accumulate MAE, MSE
        mae1 = abs(predict_cnt_1 - gt_cnt_1)
        mae2 = abs(predict_cnt_2 - gt_cnt_2)
        #mse = (predict_cnt - gt_cnt) * (predict_cnt - gt_cnt)
        #Changed mse here to mae to keep track of the overall mae while still using mae for monitoring cmae
        mse =abs(predict_cnt - gt_cnt)
        maes1.append(float(mae1))
        maes2.append(float(mae2))
        mses.append(float(mse))
    # calc MAE, MSE
    mae = (np.mean(maes1) + np.mean(maes2))/2
    mse = np.mean(mses)
