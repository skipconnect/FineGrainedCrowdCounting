...
class AnchorPoints(nn.Module):
    def __init__(self, pyramid_levels=None, strides=None, row=3, line=3,cpu_test=False):
    ...
    def forward(self, image):
        ...
        if torch.cuda.is_available() and not self.cpu_test:
...
class P2PNet(nn.Module):
    def __init__(self, backbone, row=2, line=2, cpu_test=False):
        super().__init__()
        ...
        self.anchor_points = AnchorPoints(pyramid_levels=[3,], row=row, line=line, cpu_test=cpu_test)
...
def build(args, training):
    ...
    model = P2PNet(backbone, args.row, args.line, args.cpu_test)
    if not training: 
        return model
