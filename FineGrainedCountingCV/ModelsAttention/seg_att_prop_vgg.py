...
from ModelsAttention.networks import HOUR_GLASS_PROP, CSRNet_TWO, SpatialAggregator
...
class SegAttPropVGG():
    def __init__(self, opt):
        ...
        if opt.net == 'vgg':
            ...
            #self.seg_refiner_att = HOUR_GLASS_PROP(64, opt.output_cn+1, _iter=opt.hourglass_iter, att_cn=att_cn, prop=opt.prop).cuda()
            self.seg_refiner_att= SpatialAggregator(dim=65, dim_index=1, heads=5, num_dimensions=2, n_att_layers=4, o_cn=opt.output_cn+1,  _iter=opt.hourglass_iter).cuda()
            #self.cnt_refiner_att = HOUR_GLASS_PROP(64, 1, _iter=1, att_cn=att_cn).cuda()
            self.cnt_refiner_att= SpatialAggregator(dim=65, dim_index=1, heads=5, num_dimensions=2, n_att_layers=4, o_cn=1,  _iter=1).cuda()
        else:
        ...
        params = list(self.seg_refiner_att.parameters()) + list(self.cnt_refiner_att.parameters())
        ...
        for name, p in self.init_predictor.named_parameters():
            print(name)
        for name, p in self.seg_refiner_att.named_parameters():
            print(name,p.device)
        for name, p in self.cnt_refiner.named_parameters():
            print(name)
            
        sum1 = sum(p.numel() for p in self.init_predictor.parameters() if p.requires_grad)
        sum2 = sum(p.numel() for p in self.seg_refiner.parameters() if p.requires_grad)
        sum3 = sum(p.numel() for p in self.cnt_refiner.parameters() if p.requires_grad)
       
        
        sum2_att = sum(p.numel() for p in self.seg_refiner_att.parameters() if p.requires_grad)
        sum3_att = sum(p.numel() for p in self.cnt_refiner_att.parameters() if p.requires_grad)
        print("BANAN")
        print("Init_params:", sum1)
        print("seg_refiner_params:", sum2)
        print("cnt_refiner_params:", sum3)
        print("seg_refiner_att_params:", sum2_att)
        print("cnt_refiner_att_params:", sum3_att)
        print("Total:", sum1+sum2+sum3)
        print("Total_att:", sum1+sum2_att+sum3_att)
        '''
    ...
    def forward(self, no_grad=True):
        ...
        self.smap=self.seg_refiner_att(self.init_predictor.smap_fea, _dmap) #DEN HER ER ATTENTION
        ...
        self.dmap=self.cnt_refiner_att(self.init_predictor.dmap_fea, _smap)#DEN HER ER ATTENTION
    ...
    def validate(self, data):
        ...
        relative_mae = []
       # print(self.path)
       # torch.save(self.target, "./TestResults/Target.pt")
        #torch.save(output, "./TestResults/Output.pt")
        #torch.save(self.img, "./TestResults/Image.pt")
    ...
    def save(self, suffix):
        ...
        torch.save({
            ...
            'state_dict_seg_refiner_att': self.seg_refiner_att.state_dict(),
            'state_dict_cnt_refiner_att': self.cnt_refiner_att.state_dict(),
            }, path)
    ...
    def load(self, suffix):
        ...
        self.seg_refiner_att.load_state_dict(checkpoint['state_dict_seg_refiner_att'])
        self.cnt_refiner_att.load_state_dict(checkpoint['state_dict_cnt_refiner_att'])
        print("Loaded model: %s" % path)
